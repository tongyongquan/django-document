...
try:
    ...
    hot_articles = Article.objects.filter().order_by('-views')[:10]
    # months = {}
...
def home(request):  # 主页
    ...
    return render(request, 'home.html',
                  {'post_list': post_list, 'category_list': categories, 'hot_articles': hot_articles, 'tag_list': tags})
...
def detail(request, id):  # 查看文章详情
    try:
        ...
        post.viewed()  # 更新浏览次数
        article_tags = post.tags.all()  # 获取文章对应所有标签
    ...
    return render(request, 'post.html',
                  {
                      'post': post,
                      'category_list': categories, 'hot_articles': hot_articles, 'tag_list': tags,
                      'tags': article_tags,
                      'next_post': next_post,
                      'prev_post': prev_post
                  })
